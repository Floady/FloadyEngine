...
FD3d12Quad::FD3d12Quad(UINT width, UINT height)
	: m_viewport(),
	m_scissorRect()
{
...
void FD3d12Quad::Init(ID3D12CommandAllocator* aCmdAllocator, ID3D12Device* aDevice, D3D12_CPU_DESCRIPTOR_HANDLE& anRTVHandle, ID3D12CommandQueue* aCmdQueue, ID3D12DescriptorHeap* anSRVHeap, ID3D12RootSignature* aRootSig)
{
	m_device = aDevice;
	HRESULT hr;
	...
	// Create the vertex buffer.
	{
		...
		// Create the texture.
		{
			...
			CD3DX12_CPU_DESCRIPTOR_HANDLE srvHandle0(anSRVHeap->GetCPUDescriptorHandleForHeapStart(), 1, srvSize);
...
void FD3d12Quad::Render(ID3D12Resource* aRenderTarget, ID3D12CommandAllocator* aCmdAllocator, ID3D12CommandQueue* aCmdQueue, D3D12_CPU_DESCRIPTOR_HANDLE& anRTVHandle, ID3D12DescriptorHeap* anSRVHeap)
{
	...
	handle.ptr += srvSize; // tried to increase the SRV heap ptr here to get the right tex.. didnt work
