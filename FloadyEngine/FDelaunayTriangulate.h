...
public:
	...
	void GenerateMesh(FVector3 aMin, FVector3 aMax);
	bool IsBlocked(int aTriangle) { return aTriangle >= 0 ? myBlockedTriangleList[aTriangle] : false; }
	...
	struct AABB
	{
		FVector3 myMin;
		FVector3 myMax;
		bool IsInside(FVector3 aPoint)
		{
			// 2d
			return aPoint.x >= myMin.x && aPoint.x <= myMax.x && aPoint.z >= myMin.z && aPoint.z <= myMax.z;
		}
	};
...
private:
	...
	FVector3 GetCenter(int aTriangleIdx);
	float GetDistanceTriangleToPoint(int aTriangleIdx, FVector3 aPos);
	FVector3 GetClosestPointOnTriToPointAllowedTri(int aTriangleIdx, FVector3 aPos, int aFromTriangle);
